...
** What's [[https://nixos.org/][Nix]]?
- Declarative package manager
- Project dependencies declared in =flake.nix=
- Guarantees identical environments across machines
- [[https://github.com/nix-community/nix-direnv][nix-direnv]] enables automatic environment switching

** Installation
*** [[https://nixos.org/][Nix]] package manager
- Download =.pkg= from [[https://docs.determinate.systems/][Determinate Systems]]
  - This method survives [[https://www.apple.com/macos/][macOS]] system upgrades

*** [[https://direnv.net/][Direnv]] + [[https://github.com/nix-community/nix-direnv][nix-direnv]]
- Install [[https://direnv.net/][direnv]]: =brew install direnv=
- Install [[https://github.com/nix-community/nix-direnv][nix-direnv]]: =nix profile add nixpkgs#nix-direnv=
- Configure =~/.config/direnv/direnvrc=
- Hook into shell
  - [[https://fishshell.com/][Fish]]: =direnv hook fish | source=
  - [[https://www.zsh.org/][Zsh]]: =eval "$(direnv hook zsh)"=
- Reload shell config
...
*** Create =flake.nix=
- Use text editor
- Three parts: description, inputs, outputs
- Inputs: declare external flake dependencies
- Outputs: define what the flake provides (dev environment, packages, tools)

*** Auto-activate environment
- Create =.envrc=: =echo "use flake" > .envrc=
- Trust directory: =direnv allow=
- Nix generates =flake.lock= (pins versions)
