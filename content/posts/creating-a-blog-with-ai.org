#+TITLE: Hello World
#+DATE: 2025-12-28
#+DESCRIPTION: Building a self-contained blog engine with Rust, Axum, and org-mode using Claude Code. Complete walkthrough of deployment, features, and lessons learned from AI-assisted development.
...
Rate limiting protects the site using the [[https://github.com/benwis/tower-governor][tower-governor]] crate,
which applies limits based on the =X-Forwarded-For= header to properly
handle requests coming through [[https://www.cloudflare.com/][Cloudflare]] and the [[https://caddyserver.com/][Caddy]] reverse proxy.

Posts automatically display last updated timestamps extracted from [[https://git-scm.com/][git]] commit history - a build script pulls the last commit date for each post and embeds it at compile time, only showing the updated date when it differs from the publication date.
...
The complete source code is available on [[https://github.com/spwx/blog][GitHub]].
...
Version upgrades were surprisingly difficult. When I asked Claude Code
to upgrade to a major version of [[https://github.com/djc/askama][Askama]], it got really confused about
some pretty simple things. It couldn't figure out basic [[https://github.com/djc/askama][Askama]] usage
patterns until I explicitly explained how the templating worked. This
was strange because it had been working with [[https://github.com/djc/askama][Askama]] successfully
earlier.

When adding rate limiting, Claude Code hallucinated and used a
completely wrong method. This was the only straight up hallucination I
saw during the project.

Claude Code's world knowledge is outdated. It tried to use
[[https://crates.io/crates/once_cell][once_cell]] instead of the built-in [[https://doc.rust-lang.org/std/sync/struct.LazyLock.html][std::sync::LazyLock]] that's now
available in standard [[https://www.rust-lang.org/][Rust]]. I had to prompt it to fix this.
...
Starting with the latest versions of libraries would avoid upgrade
problems down the line. The version upgrade difficulties showed that
Claude Code handles greenfield development better than migrating
existing code to new major versions.

* Using The Blog Engine
The blog engine is simple to use once it's set up. To run the local
development server, just use:

#+begin_src sh
just run
#+end_src

This starts the server locally so you can preview changes before
deploying.
...
This configures the server infrastructure including [[https://caddyserver.com/][Caddy]], [[https://systemd.io/][systemd]]
services, and required directories. After setup is complete, deploy to
the server with:
...
The [[https://just.systems/][Just]] command runner handles all the compilation and deployment
steps.
...
- Rate limiting - Implemented with [[https://github.com/benwis/tower-governor][tower-governor]] using
  =X-Forwarded-For= header for proper client identification behind
  reverse proxy
